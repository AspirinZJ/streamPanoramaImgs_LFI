...
static void help()
{
	cout << "This is a camera calibration sample.\n"
		 << "Usage: calibrateCam [configuration_file -- default ./default.xml]\n"
...
public:
	...
	void validate()
	{
		...
		if (input.empty()) inputType = INVALID;
		else
		{
			if (input[0] >= '0' && input[0] <= '9') // camera input
			{
		...
		if ("CHESSBOARD" == patternToUse) calibrationPattern = CHESSBOARD;
		if ("CIRCLES_GRID" == patternToUse) calibrationPattern = CIRCLES_GRID;
		if ("ASYMMETRIC_CIRCLES_GRID" == patternToUse) calibrationPattern = ASYMMETRIC_CIRCLES_GRID;
	...
	static bool readStringList(const string &filename, vector<string> &vec)
	{
		vec.clear();
		...
		for (; it != it_end; ++it) vec.push_back((string) *it);

		return true;
	}

	// 检查文件名是否是以xml,yaml或yml结尾
	static bool isListOfImages(const string &filename)
	{
		const string &s(filename);
		// Look for file extension
		// if string find() method cannot find, it returns -1(string::npos)
...
int main(int argc, char *argv[])
{
	...
	// * Get next input, if it fails or have enough of them -> calibrate
	for (;;)
	{
		Mat view;
		...
		{
			if (runCalibrationAndSave(s, imageSize, cameraMatrix, distCoeffs, imagePoints)) mode = CALIBRATED;
			else mode = DETECTION;
		}
		...
		if (mode == CAPTURING)
		{
			if (s.showUndistorted) msg = format("%d/%d Undist", (int) imagePoints.size(), s.nrFrames);
			else msg = format("%d/%d", (int) imagePoints.size(), s.nrFrames);
		}
		...
		{
			Mat temp = view.clone();
			if (s.useFisheye) fisheye::undistortImage(temp, view, cameraMatrix, distCoeffs);
			else undistort(temp, view, cameraMatrix, distCoeffs);
		}
		//! [output_undistorted]
		...
		else
		{
			initUndistortRectifyMap(cameraMatrix, distCoeffs, Mat(),
									getOptimalNewCameraMatrix(cameraMatrix, distCoeffs, imageSize, 1, imageSize,
															  nullptr), imageSize, CV_16SC2, map1, map2);
		}

		for (auto &i : s.imageList)
		{
			view = imread(i, IMREAD_COLOR);
...
//! [compute_errors]
static double
computeReprojectionErrors(const vector<vector<Point3f>> &objectPoints, const vector<vector<Point2f>> &imagePoints,
						  ...
						  const Mat &distCoeffs, vector<float> &perViewErrors, bool fisheye)
{
	...
	for (size_t i = 0; i < objectPoints.size(); ++i)
	{
		...
		else { projectPoints(objectPoints[i], rvecs[i], tvecs[i], cameraMatrix, distCoeffs, imagePoints2); }
...
//! [board_corners]
static void calcBoardCornerPositions(const Size &boardSize, float squareSize, vector<Point3f> &corners,
		...
		case Settings::CIRCLES_GRID:
			for (int i = 0; i < boardSize.height; ++i)
				for (int j = 0; j < boardSize.width; ++j)
					...
					// use emplace_back for non-built-in types and pass the parameters directly to the type constructor
						   ...
						   vector<float> &reprojErrs, double &totalAvgErr)
{
	...
	//! [fixed_aspect]
	if (s.useFisheye) { distCoeffs = Mat::zeros(4, 1, CV_64F); }
	else { distCoeffs = Mat::zeros(8, 1, CV_64F); }
							 ...
							 const vector<vector<Point2f>> &imagePoints, double totalAvgErr)
{
	...
	if (!rvecs.empty() || !reprojErrs.empty()) fs << "nr_of_frames" << (int) max(rvecs.size(), reprojErrs.size());
